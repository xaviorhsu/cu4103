...
function OCR_window(w_func,w_url,w_name,w_width,w_height) {  
  switch(w_func) {
    case 'O': //開啟置中子視窗
        var w_features="",xposition=15,yposition=25;
        if(w_width!=0||w_height!=0)
          {
            if(window.screen) //取得螢幕解析度
                {
                  xposition=(screen.width-w_width)/2;           //窗體居中的x坐標
                  yposition=(screen.availHeight-w_height)/2;    //可視窗體居中的y坐標
                }               
            //打開視窗的屬性
...
function disp_handle(id,item,last_item,tpe)
  {	if ($("#phod2").is(":visible")){ $("#SF2").attr("src",""); }
    for (var i=0; i<=last_item; i++) { document.getElementById(id+i).style.display = 'none'; }
    document.getElementById(id+item.substr(0,1)).style.display = 'block'; 
    if(tpe==1) document.getElementById(id+item).checked = true;
  }
// ----------------------------------------------------------------------------------- //
//            產生與變更視框窗內檔
...
function set_scrfile(id,scrname)
  {	if ($("#phod2").is(":visible")){ $("#SF2").attr("src",""); }
    ...
    if(scrname && id!="SF2")  document.getElementById(id).innerHTML = '<iframe name="myiframe" align="center" src="'+scrname+'" style="width:97.5%;height:345px;margin-left:5px;background-color:#FFEFDF;"/></iframe>';
	else $("#SF2").attr("src","https://www.youtube.com/embed/"+scrname);
  }
// ----------------------------------------------------------------------------------- //
//            產生與變更區塊內圖檔
...
function set_imgfile(id,FTcolor,titlename,imgname)
  {    
    ...
    if(imgname)  document.getElementById(id).innerHTML = '<div align="center" style="width:98%;color:'+FTcolor+';font-size:22px;font-weight:bold;font-family:微軟正黑體,新細明體;"><b>'+titlename+'</b><br /><img border="1" src="'+imgname+'" style="margin:10px 0 0 4px;width:523px;height:305px;"/></div>';
  }
// ----------------------------------------------------------------------------------- //
//            按鈕鍵只能按一次，按過後變灰色
...
//參數total表示總共要顯示商品照片圖的張數，參數id1表示要顯示商品照片圖總頁數位置的元素ID名
// ====> 須要 globalvar idx=1;
...
function chk_form(mc,id)
{
  ...
  for (var i=0;i<id.length;i++) {
    ...
    switch(id[i]) { 
      ...
      case "xcustid":
      case "xcusti2":          
          if(doc_x.value=='身分證號/公司統編'||((doc_x.value).length!=10&&(doc_x.value).length!=8)||(((doc_x.value).length==10&&!check_TWid(doc_x.value))||((doc_x.value).length==8&&!check_TB(doc_x.value))))
...
// --------------------------------------------------------------------------------------------------- //      
      ...
      case "xname":
      case "xnam2":
           if(doc_x.value==''){$('#'+id[i]).addClass("pdata"); sw=1; err_msg+='● 請輸入姓名\n';}
           else {str+=","+doc_x.value;}
      break;
      case "xmarry":
          if(!doc_m.checked && !doc_c.checked)
      ...
      case "xtelephone":
      case "xtelephon2":
           if(doc_x.value==''){$('#'+id[i]).addClass("pdata"); sw=1; err_msg+='● 請填寫聯絡電話\n';}
           else {str+=","+doc_x.value;}
      break;
      case "xcellphone":
      case "xcellphon2":
           if(doc_x.value==''){$('#'+id[i]).addClass("pdata"); sw=1; err_msg+='● 請填寫行動電話\n';}
           else {str+=","+doc_x.value;}
      break;
      case "xemail":
      case "xemai2":
      ...
      case "xaddres1":
      case "xaddres2":
           if(doc_x.value==''){$('#'+id[i]).addClass("pdata"); sw=1; err_msg+='● 請填寫聯絡地址\n';}
           else {
              if (id[i]=="xaddres1") j=1; if (id[i]=="xaddres2") j=2;
              t_addr=$('#PostNo'+j).val()+$('#City'+j).val()+$('#Canton'+j).val();
...
// --------------------------------------------------------------------------------------------------- //
      case "xpaytype":
           ...
           if(!doc_m.checked&&!doc_c.checked&&!doc_d.checked){ $('#s4I1,#s4I2,#s4I3').addClass("pdata");sw=1; err_msg+='● 請選一種繳費方式\n';}
   ...
   if(sw){ alert(err_msg); str=''; return str; }
   else { str+='"'; return str;}
}
//------------------------------------------------------------------------------------------ //
...
//-----------------------------------------------------------------------------------------------
  function callAjaxDo(id,sw,url,options) {
    $("#"+id).html(); //empty content
    switch(sw)  {        
        case "g" :  //GET方式
                    show_succ(id);
            				xmlHttp.open('GET',url+"?"+options,true);
            				xmlHttp.send(null);				
        break;
        case "p" :  //POST方式
                    show_succ(id);
                    xmlHttp.open('POST',url, true);
            				...
            				xmlHttp.send(options);  //這裡面指定要POST的值
        break;
        case "j" :  //$.getJSON方式 jsonp
                    var data = $.getJSON(url,options);
                    //成功得到資料
                    data.success(function(data) { $.each(data, function(key,val){                    
                        counters=(key=="Ans")?val:"F_ERROR_F"; $("#"+id).html(counters);
                        });
                    });
                    //取得資料失敗
                    data.error(function(data) { $("#"+id).html("fail getting data"); });
        break;
        case "s" :  //$.ajax方式 JSONP（JSON with Padding）
                    $.ajax({                                
                          url        : url,
                          type       : 'GET',
                          data       : options,
                          ...
                          crossDomain: true,
                          //成功得到資料
                          success    : function(data,textStatus,jqXHR){ $.each(data, function(key,val)
                                       { counters=(key=="Ans")?val:"F_ERROR_F"; $("#"+id).html(counters); }
                                       )},
                          //取得資料失敗
